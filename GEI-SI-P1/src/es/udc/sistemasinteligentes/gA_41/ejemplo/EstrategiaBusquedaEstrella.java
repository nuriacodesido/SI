...
import es.udc.sistemasinteligentes.gA_41.*;
            ...
            if(frontera!=null){//Si la frontera no está vacía.
                ...
                if(!p.esMeta(nodo.getEstado())) {
                    ...
                    for (Nodo suc : sucesores) { //Para cada sucesor
                        suc.setCoste(suc.getPadre().getCoste()+1);
                        suc.setF(suc.getCoste() + ((int) (h.evalua(suc.getEstado()))));
                        if (!explorados.contains(suc.getEstado())) {
                            if (!frontera.contains(suc.getEstado())) {
                                frontera.add(suc);
                                System.out.println((i++) + " - Añadimos el nodo a frontera, con estado: " + suc.getEstado() + " , coste: " + suc.getCoste() + " , heuristica: " + h.evalua(suc.getEstado()) + " , f(n): " + suc.getF() + "  -");

                            } else {
                                if ((suc.getF()) < ((frontera.get(0)).getF())) {
                                    frontera.remove(frontera.get(0));
                                    frontera.add(suc);
                                    System.out.println((i++)+ " - Añadimos el nodo con estado: " + suc.getEstado() +" , coste: " + suc.getCoste() + " , heuristica: " + h.evalua(suc.getEstado()) + " , f(n): " +suc.getF() + "  -");
                                }
                            }
                        }
                    }
